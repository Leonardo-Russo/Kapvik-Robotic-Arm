function [Q] = invkinAnal(X, L1, L2, typeOfSol)
...
if abs(prec)<10^(-4) || abs(prec-pi)<10^(-4)|| abs(prec+pi)<10^(-4) % sin(yaw) circa 0, uso l'elemento della matrice 1,4 piuttosto che 2,4
    c2(1)=(L1*x + L2*c3*x + L2*s3(1)*z*cos(prec))/...
        (cos(prec)*(L1^2 + 2*L1*L2*c3 + L2^2*c3^2 + L2^2*s3(1)^2)); % elbow up
    s2(1)=(L1*z*cos(prec) - L2*s3(1)*x + L2*c3*z*cos(prec))/...
        (cos(prec)*(L1^2 + 2*L1*L2*c3 + L2^2*c3^2 + L2^2*s3(1)^2)); % elbow up
    c2(2)=(L1*x + L2*c3*x + L2*s3(2)*z*cos(prec))/...
        (cos(prec)*(L1^2 + 2*L1*L2*c3 + L2^2*c3^2 + L2^2*s3(2)^2)); % elbow down
    s2(2)=(L1*z*cos(prec) - L2*s3(2)*x + L2*c3*z*cos(prec))/...
        (cos(prec)*(L1^2 + 2*L1*L2*c3 + L2^2*c3^2 + L2^2*s3(2)^2)); % elbow down
else
    c2(1)=(L1*y + L2*c3*y + L2*s3(1)*z*sin(prec))/...
        (sin(prec)*(L1^2 + 2*L1*L2*c3 + L2^2*c3^2 + L2^2*s3(1)^2)); % elbow up
    s2(1)=(L1*z*sin(prec) - L2*s3(1)*y + L2*c3*z*sin(prec))/...
        (sin(prec)*(L1^2 + 2*L1*L2*c3 + L2^2*c3^2 + L2^2*s3(1)^2)); % elbow up
    c2(2)=(L1*y + L2*c3*y + L2*s3(2)*z*sin(prec))/...
        (sin(prec)*(L1^2 + 2*L1*L2*c3 + L2^2*c3^2 + L2^2*s3(2)^2)); % elbow down
    s2(2)=(L1*z*sin(prec) - L2*s3(2)*y + L2*c3*z*sin(prec))/...
        (sin(prec)*(L1^2 + 2*L1*L2*c3 + L2^2*c3^2 + L2^2*s3(2)^2)); % elbow down
end
